...
**KOSIS 작업을 시작하면 가장 먼저 [LEARNINGS.md](LEARNINGS.md)를 읽는다.** 과거의 실패·함정·성공 쿼리가 누적된 "오답노트"다.

- **조회 전 READ**: `LEARNINGS.md`의 `테이블별 함정`·`오류코드 대응`을 확인해 같은 실수를 반복하지 않는다.
...
```bash
kosis update          # 최신 버전으로 업데이트 (바이너리 + 스킬 파일 함께)
kosis update --check  # 업데이트 확인만 (설치 안 함)
kosis update --force  # 같은 버전이어도 강제 재설치
```

GitHub 최신 릴리스에서 OS별 바이너리와 스킬 파일(SKILL.md, references/, LEARNINGS.md, templates 등)을 함께 내려받아 `~/.claude/skills/kosis`(및 `~/.codex/skills/kosis`)에 반영합니다. macOS/Linux/Windows 모두 지원합니다.
...
# make native   # 현재 플랫폼(darwin/arm64)만 빠르게
```

> 정식 완전 바이너리(SQLite 포함)는 CI(`.github/workflows/release.yml`, CGO=1)가 생성한다.
> 로컬 `make build`는 네이티브(darwin/arm64)만 CGO=1이고 나머지는 CGO=0이라 SQLite 출력 기능이 제외된다.
...
## 통계표 탐색 전략 (AI 필독 — API 검색보다 빠른 길)

원하는 통계표의 ORG/TBL을 찾을 때는 **아래 순서**로 시도한다. 로컬 파일이 API 왕복+동의어 재시도보다 훨씬 빠르다.

```bash
# 0순위: 아래 "자주 쓰는 통계표" 치트시트 확인 (grep조차 불필요)

# 1순위: 로컬 인덱스 grep — ORG/TBL 즉시 획득 (오프라인, 5,128개 표)
grep -i "미분양" references/kosis-table-index.md
# → | 116 | DT_MLTM_2086 | 미분양주택현황 | 주거 > ... |

# 2순위: 파라미터 가이드 grep — 표당 한 줄에 분류/항목/주기 코드가 다 있어 meta 호출 생략 가능
grep "DT_MLTM_2086" references/kosis-table-params.md

# 3순위: API 검색 — 로컬 인덱스에 없을 때만
kosis s "미분양" -n 10
```

- **경로 기준**: 위 `references/`는 이 SKILL.md와 같은 디렉토리(설치 시 `~/.claude/skills/kosis/`) 기준.
- **Windows**: `grep` 대신 `Select-String -Path references\kosis-table-index.md -Pattern "미분양"` 사용.
- **⚠ 인덱스 누락 주의**: 인덱스는 주제 트리 깊이 2까지만 수집돼 **소비자물가지수·국내총생산·실업률·출생아수 등 핵심 표 다수가 빠져 있다**. grep 0건이 "표 없음"을 뜻하지 않는다 → 치트시트 확인 후 `kosis s`로 폴백. 검색도 0건이면 통계 **조사명**(예 "광업제조업조사")으로 재검색(LEARNINGS.md 참조).
- **찾은 뒤에는**: params grep으로 코드를 확보했어도 확신이 없으면 `kosis m <ORG> <TBL>`로 확정한다(인덱스 수집일 이후 개편 가능).
...
# 시군구별 성별 주민등록인구 (연간, 자동분할)
kosis d 101 DT_1B040A3 -c1 ALL -i ALL -p Y -l 5

# 시군구별 제조업 출하액 — 광업제조업조사 (T04=출하액, T06=부가가치)
kosis d 101 DT_1FS1101 -c1 ALL -c2 C -i T04 -p Y -l 1
```

### 주요 통계표 빠른 참조 (ORG/TBL 확인됨 — 조회 전 `meta`로 코드 확정)

| 주제 | ORG | TBL_ID | 통계표명 | 주기 |
|------|-----|--------|---------|------|
| 인구 | 101 | `DT_1IN1502` | 인구총조사 (성/연령별) | Y |
| 인구 | 101 | `DT_1B040A3` | 시군구별 성별 주민등록인구 | Y |
| 인구 | 101 | `DT_1B04006` | 시군구별 1세별 주민등록인구 (⚠ 와이드, 느림 — 범위 좁혀라) | Y |
| 인구동향 | 101 | `DT_1B8000G` | 출생·사망·혼인·이혼 건수 | M/Q/Y |
| 출산 | 101 | `DT_1B81A23` | 시군구별 출생아수·합계출산율 | Y |
| 사망원인 | 101 | `DT_1B34E01` | 사망원인(104항목)/성/연령별 사망자수·사망률 | Y |
| 고용 | 101 | `DT_1DA7002S` | 성/연령별 경제활동인구 | M |
| 고용 | 101 | `DT_1DA7004S` | 시도별 경제활동인구 | M |
| 실업 | 101 | `DT_1DA7102S` | 성/연령별 실업률 | M |
| 실업 | 101 | `DT_1DA7104S` | 시도/성별 실업률 | M |
| 물가 | 101 | `DT_1J22001` | 지출목적별 소비자물가지수 | M |
| 물가 | 301 | `DT_404Y014` | 생산자물가지수(기본분류) | M |
| GDP | 101 | `DT_2KAA907` | 지출항목별 국내총생산(당해년가격) | Y |
| GDP | 101 | `DT_2OENA01` | 국내총생산(OECD 비교) | Y |
| 부동산 | 116 | `DT_MLTM_2086` | 미분양주택현황 | M/Y |
| 부동산 | 408 | `DT_PLCAHTUSE` | 시군구별/용도지역별 지가변동률 | M |
| 부동산 | 390 | `DT_39002_04` | 주택매매시장 소비심리지수 | M |
| 산업 | 101 | `DT_1FS1101` | 시군구별 산업별 제조업 출하액·부가가치 | Y |
| 유통 | 101 | `DT_1KE10071` | 온라인쇼핑몰 판매매체별/상품군별 거래액 | M |
| 복지 | 117 | `DT_11714_N001` | 국민기초생활수급자 총괄(시도별) | Y |

> ⚠ 통계표마다 분류코드 형식이 다릅니다. 빠른 참조 표의 항목은 **ORG/TBL 존재만 검증**된 것이므로, 조회 전 반드시 `kosis m <ORG> <TBL>` 또는 `grep "<TBL_ID>" references/kosis-table-params.md` 로 분류/항목/주기 코드를 확인하세요.
> 검증된 새 표를 발견하면 이 표와 LEARNINGS.md에 추가하세요 (학습 루프).
...
| `31` | 40,000셀 초과 | 자동 분할로도 **단일 시점이 4만 셀 초과**면 분류(`-c1`/`-c2`)·항목(`-i`) 범위를 좁혀라 ("전체×전체×다년도" 회피) |
| HTTP 429 | 분당 호출 제한 초과 | 자동 지수 백오프 재시도 (최대 3회). 잦으면 `config add-key`로 키 추가 |
...
```
1. 치트시트 → grep 인덱스 → kosis s   → ORG_ID, TBL_ID 확인 ("통계표 탐색 전략" 참조)
...
| 문서 | 내용 |
|------|------|
| [01-installation.md](references/01-installation.md) | 설치 및 초기 설정 |
| [02-data-paths.md](references/02-data-paths.md) | 두 가지 데이터 경로 (통계표/주요지표) |
| [03-statistics-query.md](references/03-statistics-query.md) | 통계표 조회 Step by Step |
| [04-indicators.md](references/04-indicators.md) | 주요지표 조회 (search/info/data/list) |
| [05-quick-query.md](references/05-quick-query.md) | 자연어 조회, 내장 사전, AI 모드 |
| [06-list-explain-bulk.md](references/06-list-explain-bulk.md) | 통계목록 탐색, 통계설명, 대용량 다운로드 |
| [07-output-formats.md](references/07-output-formats.md) | 출력 형식 (table/JSON/CSV/Markdown/Excel/SQLite/Parquet) |
| [08-utilities.md](references/08-utilities.md) | 편의 기능 (즐겨찾기, 이력, 캐시, AI 도구 설정) |
| [09-interactive-tui.md](references/09-interactive-tui.md) | 대화형 모드 및 TUI 대시보드 (키바인딩) |
| [10-auto-split.md](references/10-auto-split.md) | 자동 분할 조회 (4만 셀 초과 시 분할/병렬 처리) |
| [11-related-tables.md](references/11-related-tables.md) | 연관 통계표 찾기 |
| [12-examples.md](references/12-examples.md) | 자주 쓰는 통계표 및 실전 예시 |
