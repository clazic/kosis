...
> ⚠ **구버전 함정**: 일부 CLI 버전은 `-f json` / `--fields "C1"`을 줘도 분류 코드가 `null`로 나온다(이름만 노출). 지도 결합이 필요한데 코드가 비면 → `meta -f json`의 CLASSIFICATIONS 역매핑(`references/16-geo-join.md` §16.3 표준 레시피)을 사용하라.
